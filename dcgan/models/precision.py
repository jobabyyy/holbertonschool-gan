...
train_images = train_images.reshape(train_images.shape[0], 28, 28, 1).astype('float16')  # Change to float16
...
def make_discriminator_model():
    ...
    model.add(layers.Conv2D(64, (5, 5), strides=(2, 2), padding='same',
                             input_shape=[28, 28, 1], dtype=tf.float16))  # Changed to float16
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))

    model.add(layers.Conv2D(128, (5, 5), strides=(2, 2), padding='same', dtype=tf.float16))  # Changed to float16
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))
...
noise = tf.random.normal([1, noise_dim], dtype=tf.float16)  # Change to float16
...
def train_step(images):
    noise = tf.random.normal([BATCH_SIZE, noise_dim], dtype=tf.float16)  # Change to float16)
